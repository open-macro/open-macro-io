...
DDo = (r_do-g)*d_o/(1+g) + (ro-g)*bo/(1+g) + (rstar+nug-g)*dco/(1+g) + P_zo*i_zo-miu*s_s*zo-(grantso + oilro);
...
f(1) = 1 + r - (1+rstar+nu+nug*exp(etag*(d/y+dc/y-d_o/yo-dco/yo)))/(1-eta*(bstar-bstaro));
f(2) = bstar + d + dc - P*c - P_k*(i_x+i_n) - P_z*i_z - (1+r_do)*d/(1+g) - (1+rstar+nug*exp(etag*(d/y+dc/y-d_o/yo-dco/yo)))*dc/(1+g) - (1+rstar+nu+nug*exp(etag*(d/y+dc/y-d_o/yo-dco/yo)))*bstar/(1+g) + y + remit + (grants+oilr);
...
f(6) = h*(e+eh) - ho*(eo+eho) - ((r_do-g)*d/(1+g) + (r-g)*P*b/(1+g) + (rstar+nug*exp(etag*(d/y+dc/y-d_o/yo-dco/yo))-g)*dc/(1+g) + P_z*i_z + T-To - miu*ze - (grants+oilr) - DDo);
f(7) = T-To + lambda*((r_do-g)*d/(1+g) + (r-g)*P*b/(1+g) + (rstar+nug*exp(etag*(d/y+dc/y-d_o/yo-dco/yo))-g)*dc/(1+g) +  P_z*i_z+ho*(e+eh-eo-eho)-miu*ze-(grants + oilr)-DDo);  
