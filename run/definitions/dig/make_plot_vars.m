...
qxlag    = [q_xo qx(1:1001)']';
qnlag    = [q_no qn(1:1001)']';
ixn      = ix+in;
rer      = pn ./ px;
wr       = w ./ p;
c        = (e+eh).*p;
...
plot_vars.pfd       = 100*(p.*(b-blag)+d-dlag+dc-dclag-(r_d-g).*dlag./(1+g)-(rextglag-g).*dclag./(1+g)-(rlag-g).*p.*blag/(1+g))./ynom;
